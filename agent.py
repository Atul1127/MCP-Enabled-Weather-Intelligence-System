...
def _render_current_weather(result: dict[str, Any]) -> str | None:
    ...
    if summary.get("time_of_day") in {"day", "night"}: parts.append(f"it is currently {summary['time_of_day']}")
    if summary.get("condition"): parts.append(f"with {summary['condition'].lower()}")
    if summary.get("temperature_c") is not None: parts.append(f"and {summary['temperature_c']}°C")
    if summary.get("apparent_temperature_c") is not None: parts.append(f"(feels like {summary['apparent_temperature_c']}°C)")
    if summary.get("relative_humidity_pct") is not None: parts.append(f"Humidity is {summary['relative_humidity_pct']}%.")
    if summary.get("cloud_cover_pct") is not None: parts.append(f"Cloud cover is {summary['cloud_cover_pct']}%.")
    if summary.get("wind_speed_kmh") is not None: parts.append(f"Wind is {summary['wind_speed_kmh']} km/h.")
    if summary.get("observation_time"): parts.append(f"Observation: {summary['observation_time']} ({summary.get('timezone')}).")
    return " ".join(parts)


def _render_forecast(result: dict[str, Any]) -> str | None:
    forecast = result.get("forecast")
    if not isinstance(forecast, dict): return None
    name = result.get("location", {}).get("display_name", "the requested location")
    return (f"Forecast for {name} on {forecast.get('date')}: {forecast.get('condition')}, " f"{forecast.get('temperature_min_c')}–{forecast.get('temperature_max_c')}°C, " f"{forecast.get('precipitation_probability_pct')}% precipitation probability, " f"{forecast.get('precipitation_mm')} mm precipitation, and maximum wind " f"{forecast.get('max_wind_kmh')} km/h.")


def _rag_sources(observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    sources: list[dict[str, Any]] = []; seen: set[str] = set()
    for observation in observations:
        if observation.get("tool") not in {"search_weather", "ask_weather"}: continue
        result = observation.get("result")
        if not isinstance(result, dict) or not result.get("success"): continue
        for source in result.get("sources") or []:
            if not isinstance(source, dict): continue
            citation = str(source.get("citation") or "").strip()
            if not citation or citation in seen: continue
            seen.add(citation); sources.append(source)
    return sources
...
def _append_rag_sources(answer: str, observations: list[dict[str, Any]]) -> str:
    ...
    if re.search(r"(?im)^\s*sources\s*:?\s*$", answer): return answer
    lines = ["", "Sources:"]
    for source in sources:
        citation = source.get("citation", ""); title = source.get("title") or source.get("source") or "Local weather knowledge base"; topic = source.get("topic"); suffix = f" — {topic}" if topic else ""
        ...
        for round_no in range(1, MAX_ROUNDS + 1):
            ...
            if not calls:
                if retrieval_failure:
                    ...
                    answer = _append_rag_sources((response.message.content or "").strip(), state.observations)
            ...
            async def execute(call: Any):
                ...
                try:
                    with span("tool." + name, trace_id=state.trace_id, tool=name) as info:
                        result = await call_tool(session, name, args) if name in ALLOWED_TOOLS else {"success": False, "error": f"Tool '{name}' is not allowed."}
                        info["success"] = bool(result.get("success", True)) if isinstance(result, dict) else True
                except Exception as exc:
                    result = {"success": False, "error": str(exc)}
                return name, args, result

            with span("agent.execute_tools", trace_id=state.trace_id, round=round_no) as info:
                results = await asyncio.gather(*(execute(call) for call in calls)); info["executed"] = len(results)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the local MCP weather agent."); parser.add_argument("query", nargs="*"); args = parser.parse_args(); prompt = " ".join(args.query).strip() or input("Weather question: ").strip(); result = asyncio.run(run_agent(prompt)); print(result["answer"]); print(f"\ntrace_id={result['trace_id']} rounds={result['rounds']}")
