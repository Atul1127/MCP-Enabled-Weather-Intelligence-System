...
# Keep the RAG model aligned with the local agent default. Override with
# WEATHER_LLM_MODEL when a smaller/larger local model is preferred.
...
VECTOR_CANDIDATES = int(os.environ.get("WEATHER_VECTOR_CANDIDATES", "20"))
BM25_CANDIDATES = int(os.environ.get("WEATHER_BM25_CANDIDATES", "20"))
...
def retrieve_vector(query: str, top_k: int = VECTOR_CANDIDATES) -> list[dict[str, Any]]:
    ...
    rows = lakebase.run_query(sql, (vector_literal, top_k))
    results = []
    for row in rows:
...
def retrieve_bm25(query: str, limit: int = BM25_CANDIDATES) -> list[dict[str, Any]]:
    ...
    rows = lakebase.run_query(corpus_sql, (BM25_CORPUS_LIMIT,))
    if not rows:
        return []
...
def reciprocal_rank_fusion(
    ...
    return results[:max(1, min(20, int(top_k)))]


def retrieve_weather(query: str, top_k: int = DEFAULT_TOP_K) -> list[dict[str, Any]]:
    """Hybrid weather retrieval using dense vectors, BM25, and RRF."""
    top_k = max(1, min(20, int(top_k)))
    vector_results = retrieve_vector(query, VECTOR_CANDIDATES)
    bm25_results = retrieve_bm25(query, BM25_CANDIDATES)
    return reciprocal_rank_fusion(vector_results, bm25_results, top_k)


def build_context(documents: list[dict[str, Any]]) -> tuple[str, list[dict[str, Any]]]:
    ...
    for index, document in enumerate(documents, start=1):
        ...
        state = document.get("state") or "Unknown state"
        district = document.get("district") or "Unknown district"
        ...
        forecast_date = document.get("forecast_date") or "unknown"
        severity = document.get("severity") or "unknown"
...
Answer questions using ONLY the weather information provided in the retrieved context.

Rules:
1. Do not invent weather facts.
2. Do not use outside knowledge to fill missing weather information.
3. If the retrieved context is insufficient, explicitly say that the available weather data is insufficient.
4. Cite factual claims using source identifiers such as [S1], [S2], etc.
5. Prefer recent information when multiple weather records are available.
6. Clearly distinguish current conditions from forecasts.
7. Do not describe an Open-Meteo condition severity as an official government warning.
8. Do not claim that a condition is an official IMD warning unless the retrieved source explicitly says so.
9. Keep answers concise and useful.
10. Use Celsius and kilometres per hour unless the retrieved data specifies otherwise.
""".strip()
...
Answer the user's question using ONLY the retrieved weather context.
...
def answer_weather_question(query: str, top_k: int = DEFAULT_TOP_K) -> dict[str, Any]:
    """Execute the complete hybrid RAG pipeline."""
    if not query or not query.strip():
        raise ValueError("Query cannot be empty.")

    documents = retrieve_weather(query.strip(), top_k)
    context, sources = build_context(documents)

    if not documents:
        return {
            "answer": "The available weather knowledge base does not contain enough information to answer this question.",
            "sources": [],
