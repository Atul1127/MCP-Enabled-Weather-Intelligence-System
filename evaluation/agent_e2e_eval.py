...
def evaluate_case(
    ...
    tool_recall = (
        len(successful & expected_tools) / len(expected_tools)
        if expected_tools
        else 1.0
    )
    ...
    arg_accuracy = (
        sum(
            _argument_match(item, result.get("tool_calls", []))
    ...
    return {
        ...
        "argument_accuracy": (
            round(arg_accuracy, 4)
            if arg_accuracy is not None
            else None
        ),
        "evidence_sufficient": bool(
            verification.get("sufficient", False)
        ),
...
async def _run() -> dict[str, Any]:
    ...
    for case in cases:
        ...
        row["infrastructure_failure"] = failure_type in {
            "quota_exhausted",
    ...
    first_failure = next(
        (
            {
                "id": row["id"],
                "exception": row["exception_message"],
                "failure_type": row["exception"],
            }
            for row in rows
    ...
    for category in sorted(
        ...
        category_summary[category] = {
            "cases": len(group),
            "success_rate": round(
                sum(row["success"] for row in group)
                / len(group),
                4,
            ),
            "tool_selection_recall": round(
                statistics.mean(
                    row["tool_selection_recall"]
                    for row in group
                ),
                4,
            ),
    ...
    summary = {
        ...
        "answer_rate": round(
            sum(row["answer_present"] for row in rows)
            / len(rows),
            4,
        ) if rows else 0.0,
        "mean_tool_selection_recall": _mean(
            rows,
            "tool_selection_recall",
        ),
        "mean_argument_accuracy": _mean(
            rows,
            "argument_accuracy",
        ),
        "evidence_sufficiency_rate": round(
        ...
        "infrastructure_failure_rate": round(
            infrastructure_failures / len(rows),
            4,
        ) if rows else 0.0,
        "agent_failure_rate": round(
            agent_failures / len(rows),
            4,
        ) if rows else 0.0,
    ...
    payload = {
        ...
        "metric_note": (
            "Tool recall measures whether expected capability "
            "families were successfully invoked; argument accuracy "
            "checks only explicitly labeled required arguments. "
            "Infrastructure failures are reported separately from "
            "agent failures."
        ),
        "summary": summary,
        "rows": rows,
