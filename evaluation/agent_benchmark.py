"""Evaluate the actual MCP weather agent's tool selection and arguments.

Run from the repository root:
    python evaluation/agent_benchmark.py

This is an end-to-end benchmark: each case calls run_agent(), so the metrics
measure the real Ollama + MCP tool-selection loop. Knowledge Graph is excluded.
"""
...
def evaluate_case(case: dict[str, Any], result: dict[str, Any], latency_ms: float) -> dict[str, Any]:
    ...
    # A case is tool-correct when every required tool instance can be matched
    # to an actual call. For alternative RAG tools, either search_weather or
    # ask_weather is acceptable.
    ...
    for expected in required_args:
        ...
        found = None
        for i in unmatched:
            if i in matched_indices or call_names[i] != tool:
                continue
            if arg_match(calls[i].get("arguments") or {}, expected):
                found = i
                break
    ...
    if not required_args:
        # RAG cases accept either retrieval tool and require at least one call.
    ...
    tool_selection_correct = matched_required == expected_count
    unnecessary_calls = max(0, len(calls) - expected_count)
    unexpected_tools = [name for name in call_names if name not in expected_tools]
    ...
    return {
        ...
        "argument_accuracy": round(argument_correct / expected_count, 4),
        "required_tools_matched": matched_required,
        "required_tools": expected_tools,
        "actual_tools": call_names,
...
async def main() -> None:
    ...
    summary = {
        "cases": len(rows),
        "tool_selection_accuracy": round(statistics.mean(r["tool_selection_correct"] for r in rows), 4),
        "argument_accuracy": round(statistics.mean(r["argument_accuracy"] for r in rows), 4),
        "agent_success_rate": round(statistics.mean(r["success"] for r in rows), 4),
        "unnecessary_tool_call_rate": round(sum(r["unnecessary_tool_calls"] for r in rows) / max(1, sum(len(r["actual_tools"]) for r in rows)), 4),
        "unexpected_tool_calls": sum(len(r["unexpected_tools"]) for r in rows),
        "mean_rounds": round(statistics.mean(r["rounds"] or 0 for r in rows), 3),
    ...
    for category in sorted({r["category"] for r in rows}):
        ...
        category_metrics[category] = {
            "cases": len(subset),
            "tool_selection_accuracy": round(statistics.mean(r["tool_selection_correct"] for r in subset), 4),
            "argument_accuracy": round(statistics.mean(r["argument_accuracy"] for r in subset), 4),
            "success_rate": round(statistics.mean(r["success"] for r in subset), 4),
        }
    ...
    report = {
        ...
        "failures": [r for r in rows if not r["tool_selection_correct"] or r["argument_accuracy"] < 1 or r["unexpected_tools"]],
        "rows": rows,
        "notes": [
            "Measures the actual local Ollama + MCP agent loop, not a mocked planner.",
            "RAG cases accept either search_weather or ask_weather.",
            "Activity argument matching allows partial token overlap to avoid penalizing harmless phrasing differences.",
