...
def search_weather(query:str,top_k:int=5,location:str|None=None,state:str|None=None)->dict[str,Any]:
    ...
    with span("mcp.search_weather",trace_id=trace_id,tool="search_weather") as info:
        result=_rag.retrieve(query.strip(),location=location,state=state);payload={"success":True,"query":query.strip(),"intent":result.plan.intent,"documents":result.documents,"context":result.context,"sources":result.sources};info["success"]=True;info["documents"]=len(result.documents);info["sources"]=len(result.sources);emit("mcp.tool.result",trace_id=trace_id,tool="search_weather",success=True,documents=len(result.documents),sources=len(result.sources));return payload

@mcp.tool()
def ask_weather(query:str,top_k:int=5,location:str|None=None,state:str|None=None)->dict[str,Any]:
    """Retrieve and return grounded weather knowledge evidence; generation is handled by the agent."""
