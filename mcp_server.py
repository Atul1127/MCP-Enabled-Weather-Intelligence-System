...
def _target_date(value: str | None, daily: dict[str, Any]) -> str:
    ...
    try:
        ...
        raise ValueError("date must be 'today', 'tomorrow', or YYYY-MM-DD") from exc

def _daily_summary(weather: dict[str, Any], target: str) -> dict[str, Any]:
    daily = weather.get("daily", {})
    dates = daily.get("time") or []
    if target not in dates:
        raise ValueError(f"No forecast available for {target}")
    ...
    return {
        ...
        "weather_code": code,
        "severity": weather_client.weather_severity(code),
        ...
        "precipitation_probability_pct": value("precipitation_probability_max"),
        "max_wind_kmh": value("wind_speed_10m_max"),
...
def get_weather(location: str) -> dict[str, Any]:
    ...
    return {
        ...
        "daily": daily,
        "forecast_summary": [_daily_summary(weather, d) for d in daily.get("time") or []],
    }
...
def get_weather_alerts(location: str) -> dict[str, Any]:
    ...
    for i, target in enumerate(daily.get("time", [])):
        ...
        apparents = daily.get("apparent_temperature_max") or []
        ...
        apparent = apparents[i] if i < len(apparents) else 0
        code = codes[i] if i < len(codes) else 0
        if probability >= 70 and rain >= 15:
            alerts.append({"date": target, "severity": "HIGH", "hazard": "Heavy rain", "probability": probability, "details": f"{rain:.1f} mm expected with {probability}% precipitation probability."})
        elif probability >= 70:
            alerts.append({"date": target, "severity": "MODERATE", "hazard": "High rain probability", "probability": probability, "details": f"Precipitation probability is {probability}%."})
        if wind >= 40:
            alerts.append({"date": target, "severity": "HIGH", "hazard": "Strong wind", "details": f"Maximum forecast wind is {wind:.1f} km/h."})
        if apparent >= 40:
            alerts.append({"date": target, "severity": "HIGH", "hazard": "Extreme heat", "details": f"Maximum apparent temperature is {apparent:.1f}°C."})
        elif apparent >= 35:
            alerts.append({"date": target, "severity": "MODERATE", "hazard": "High heat", "details": f"Maximum apparent temperature is {apparent:.1f}°C."})
        if code >= 95:
            alerts.append({"date": target, "severity": "HIGH", "hazard": "Thunderstorm", "details": f"Forecast weather code is {code}."})
...
def assess_weather_risk(location: str, activity: str = "outdoor activity", date: str = "tomorrow") -> dict[str, Any]:
    ...
    activity = activity.strip() if activity else "outdoor activity"
    if not activity:
    ...
    if not weather.get("success"):
        return weather
    target = _target_date(date, weather.get("daily", {}))
    forecast = _daily_summary(weather, target)
    rain_probability = forecast["precipitation_probability_pct"] or 0
    precipitation = forecast["precipitation_mm"] or 0
    wind = forecast["max_wind_kmh"] or 0
    apparent = forecast["apparent_temperature_max_c"] or 0
    code = forecast["weather_code"] or 0
    score = 0
    factors = []
    if rain_probability >= 70:
        score += 2; factors.append(f"high precipitation probability ({rain_probability}%)")
    elif rain_probability >= 40:
        score += 1; factors.append(f"moderate precipitation probability ({rain_probability}%)")
    if precipitation >= 10:
        score += 2; factors.append(f"heavy expected precipitation ({precipitation:.1f} mm)")
    elif precipitation >= 3:
        score += 1; factors.append(f"expected precipitation ({precipitation:.1f} mm)")
    if wind >= 30:
        score += 2; factors.append(f"strong wind ({wind:.1f} km/h)")
    elif wind >= 20:
        score += 1; factors.append(f"elevated wind ({wind:.1f} km/h)")
    if apparent >= 40:
        score += 2; factors.append(f"very high apparent temperature ({apparent:.1f}°C)")
    elif apparent >= 35:
        score += 1; factors.append(f"high apparent temperature ({apparent:.1f}°C)")
    if code >= 95:
        score += 2; factors.append("thunderstorm risk in the forecast")
    risk = "HIGH" if score >= 5 else "MODERATE" if score >= 2 else "LOW"
    recommendation = (
        f"Avoid or postpone the {activity} if possible."
        if risk == "HIGH"
        else f"The {activity} is possible with precautions and a backup plan."
        if risk == "MODERATE"
        else f"Conditions look generally suitable for the {activity}."
    )
...
def search_weather(query: str, top_k: int = 5, location: str | None = None, state: str | None = None) -> dict[str, Any]:
    ...
    with span("mcp.search_weather", trace_id="unknown", tool="search_weather") as info:
        result = _rag.retrieve(query, location=location, state=state, top_k=top_k)
        payload = {"success": True, "query": query, "intent": result.plan.intent, "documents": result.documents, "context": result.context, "sources": result.sources}
        info["success"] = True
        info["documents"] = len(result.documents)
        info["sources"] = len(result.sources)
        emit("mcp.tool.result", trace_id="unknown", tool="search_weather", success=True, documents=len(result.documents), sources=len(result.sources))
        return payload
