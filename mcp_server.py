...
def _current_time_of_day(current_time: str | None, daily: dict[str, Any]) -> str:
    ...
    try:
        current=datetime.fromisoformat(current_time); dates=daily.get("time") or []; i=dates.index(current_time[:10]) if current_time[:10] in dates else 0
        sunrise=(daily.get("sunrise") or [None])[i]; sunset=(daily.get("sunset") or [None])[i]
