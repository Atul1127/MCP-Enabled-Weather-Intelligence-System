...
class WeatherAgent:
    ...
    def __init__(self, model: str = DEFAULT_MODEL, max_rounds: int = MAX_ROUNDS, max_retries: int = MAX_RETRIES):
        ...
        self.model, self.max_rounds, self.max_retries = model, max_rounds, max_retries
        self.planner = Planner(); self.verifier = EvidenceVerifier(); self._client: genai.Client | None = None
