...
MAX_ROUNDS = max(
    1,
    int(os.environ.get("WEATHER_AGENT_MAX_ROUNDS", "4")),
)

MAX_RETRIES = max(
    0,
    int(os.environ.get("WEATHER_AGENT_MAX_RETRIES", "1")),
)
...
class WeatherAgent:
    ...
    def _client_or_raise(self) -> genai.Client:
        """Create the Gemini client lazily and validate configuration."""
        ...
        if self._client is None:
            ...
            if not key:
                raise RuntimeError(
                    "GEMINI_API_KEY is not set in the process environment"
                )
    ...
    def _declarations(
        ...
        """Convert registered MCP capabilities into Gemini declarations."""
    ...
    def _function_response_part(
        *,
        ...
        Some google-genai versions expose the optional `id` parameter on
        Part.from_function_response(), while older versions do not.

        Inspect the installed SDK at runtime and only pass `id` when the
        installed API supports it.
        """
    ...
    async def _reason(
        ...
        """Ask Gemini to select the next execution action."""

        instruction = (
            "You are the execution-selection layer. "
            "Follow the explicit plan and its execution groups. "
            "Use only the MCP capabilities exposed in the current tool "
            "declarations. Use MCP tools for live evidence and weather "
            "knowledge. Complete every required plan step before stopping. "
            "Gather every required location for comparisons. "
        ...
        if retry_reason:
            instruction += (
                "\n\nVERIFIER FEEDBACK:\n"
                f"{retry_reason}\n"
                "Correct the missing evidence by selecting appropriate "
                "MCP tools from the exposed declarations."
            )
    ...
    async def run(self, query: str) -> dict[str, Any]:
        ...
        if security["suspicious"]:
            raise ValueError(
                "Query contains a blocked prompt-injection signal"
            )
        ...
        async with connect(trace_id=trace_id) as session:
            ...
            if not declarations:
                raise RuntimeError(
                    "MCP server exposed no allowed tools"
                )
            ...
            async def planner_node(
                ...
                if (
                    state.get("intent")
                    and state["intent"] != runtime.intent
                ):
                    raise RuntimeError(
                        "Router and planner intent disagree"
                    )
                ...
                if missing_groups:
                    ...
                    raise RuntimeError(
                        "MCP plan requires at least one available "
                        f"tool from: {missing}"
                    )

                active_declarations = self._declarations(
                    registry,
                    set(route.selected),
                )
            ...
            async def synthesizer_node(
                ...
                structured_response = (
                    await synthesizer.synthesize_structured(
                        query,
                        runtime,
                    )
                )
        ...
        success = (
            runtime.required_requirements_satisfied
        )
        ...
        return {
            ...
            "confidence": structured_response.get(
                "confidence"
            ),
            "citations": structured_response.get(
                "citations",
                [],
            ),
            "warnings": structured_response.get(
                "warnings",
                [],
            ),
            "trace_id": trace_id,
