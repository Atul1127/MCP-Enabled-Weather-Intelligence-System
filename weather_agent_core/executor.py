...
class MCPExecutor:
    """Execute MCP calls through a policy boundary with bounded resilience."""
    ...
    async def _call_with_retry(self, name: str, args: dict[str, Any]) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                last_error = exc
                if attempt < self.max_retries: await asyncio.sleep(min(0.25 * (2 ** attempt), 2.0))
    ...
    async def execute(self, function_calls: list[Any]) -> list[tuple[str, dict[str, Any], Any]]:
        async def one(call: Any) -> tuple[str, dict[str, Any], Any]:
            ...
            try:
                return name, args, await self._call_with_retry(name, args)
