...
class MCPExecutor:
    ...
    async def _call_with_retry(self, name: str, args: dict[str, Any]) -> Any:
        ...
        with span("mcp.tool", trace_id=self.trace_id, tool=name, timeout_seconds=timeout) as info:
            for attempt in range(self.max_retries + 1):
                try:
                    result = await asyncio.wait_for(call_tool(self.session, name, args), timeout=timeout)
            ...
            info["attempts"] = self.max_retries + 1
            raise last_error

    @staticmethod
    def _sanitize_result(result: Any) -> Any:
        if isinstance(result, dict) and result.get("success") is False:
            return {"success": False, "error": "MCP tool reported an execution failure.", "error_type": str(result.get("error_type") or "mcp_error")}
        return result
    ...
    async def execute(self, function_calls: list[Any]) -> list[tuple[str, dict[str, Any], Any]]:
        if len(function_calls) > MAX_FUNCTION_CALLS:
            return [(str(call.name), dict(call.args or {}), {"success": False, "error": f"Too many tool calls in one round; maximum is {MAX_FUNCTION_CALLS}.", "error_type": "policy_denied"}) for call in function_calls]
